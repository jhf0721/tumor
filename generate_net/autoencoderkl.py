...
class Decoder(nn.Module):
    ...
    def __init__(
        ...
        reversed_attention_levels = list(reversed(attention_levels))#反转
...
class AutoencoderKL(nn.Module):
    ...
    def __init__(
        ...
        if isinstance(num_res_blocks, int):
            ...
            #将该值重复 len(num_channels) 次，以创建一个新的元组。
    ...
    def encode(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        z_log_var = torch.clamp(z_log_var, -30.0, 20.0)#将 z_log_var 中的所有元素值限制在 -30.0 到 20.0 的范围内。
    ...
    def sampling(self, z_mu: torch.Tensor, z_sigma: torch.Tensor) -> torch.Tensor:
        ...
        """
        eps = torch.randn_like(z_sigma)#生成一个与 z_sigma 形状相同的随机噪声向量 eps，其元素是从标准正态分布（均值为0，标准差为1）中采样的。
