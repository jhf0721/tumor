...
from .misc import  unsqueeze_right
...
class Scheduler(nn.Module):
    ...
    def __init__(self, num_train_timesteps: int = 1000, schedule: str = "linear_beta", **schedule_args) -> None:
        ...
        noise_sched = NoiseSchedules[schedule](**schedule_args)#噪声调度
    ...
    def add_noise(self, original_samples: torch.Tensor, noise: torch.Tensor, timesteps: torch.Tensor) -> tuple[torch.Tensor,torch.Tensor,torch.Tensor]:
        """
        ...
        self.alphas=self.alphas.to(device=original_samples.device, dtype=original_samples.dtype)
        self.alphas_cumprod = self.alphas_cumprod.to(device=original_samples.device, dtype=original_samples.dtype)
        #（通常是扩散模型中的累积α系数）转换（to）到与 original_samples 相同的设备和数据类型。
        timesteps = timesteps.to(original_samples.device)
#指定了目标数据类型，这保证了数值计算的一致性，避免在计算过程中发生数据类型不匹配的问题。
        sqrt_alpha_cumprod = unsqueeze_right(self.alphas_cumprod[timesteps] ** 0.5, original_samples.ndim)
        #首先计算扩散过程中累积α系数（self.alphas_cumprod）在特定时间步 timesteps 下的平方根
        #用于在张量的最右边增加一个维度。
        #original_samples.ndim 表示原始样本数据 original_samples 的维度数量。unsqueeze_right 函数确保结果 sqrt_alpha_cumprod 与 original_samples 具有相同的维度。
        ...
        sqrt_alpha=unsqueeze_right(self.alphas[timesteps]**0.5,original_samples.ndim)
        sqrt_one_minus_alpha=unsqueeze_right((1-self.alphas[timesteps])**0.5,original_samples.ndim)
        return noisy_samples,sqrt_alpha,sqrt_one_minus_alpha


    def get_velocity(self, sample: torch.Tensor, noise: torch.Tensor, timesteps: torch.Tensor) -> torch.Tensor:
        #在扩散模型中，速度的计算是逆向过程的关键步骤之一，它指导了如何在每一步中调整样本，以便逐步去除噪声并恢复出清晰的数据。
        # Make sure alphas_cumprod and timestep have same device and dtype as sample
        self.alphas_cumprod = self.alphas_cumprod.to(device=sample.device, dtype=sample.dtype)
        timesteps = timesteps.to(sample.device)
        #确保了模型中的 alphas_cumprod（累积α系数）和 timesteps（时间步）与输入样本 sample 在同一设备（如CPU或GPU）上，并且具有相同的数据类型。这是为了在进行数学操作之前保证数据的兼容性。
