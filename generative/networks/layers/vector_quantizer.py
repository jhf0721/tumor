...
class EMAQuantizer(nn.Module):
    ...
    def quantize(self, inputs: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        #self.flatten_permutation  (B,C,H,W)--->(B,H,W,C)
        #self.contiguous()  确保张量是连续的
        #self.view(-1, self.embedding_dim)  展平成(B*H*W,C) -1是一个特殊的参数，它告诉PyTorch自动计算这个维度的大小，以便保持张量的总元素数量不变。

        # Calculate Euclidean distances
        #计算输入数据 flat_input 和嵌入权重 self.embedding.weight 之间的欧几里得距离。
        distances = (
        ...
        encoding_indices = encoding_indices.view(encoding_indices_view)#将量化索引 encoding_indices 重塑回原始输入张量的形状，但去掉了通道维度。
#16 16 16
    ...
    def forward(self, inputs: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        #将原始输入张量与残差相加。在前向传播中，这实际上就是将量化操作的输出作为结果（因为残差在前向传播中不起作用）。
...
class VectorQuantizer(torch.nn.Module):
    ...
    def forward(self, inputs: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        #计算困惑度。困惑度是概率分布的指数平均值，通过取平均概率的指数来计算。
        #计算得到的 avg_probs 与自身对数的乘积的和，然后取负值，并应用指数函数 torch.exp 来得到困惑度。
