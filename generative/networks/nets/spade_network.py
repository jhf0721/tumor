...
class SPADEResNetBlock(nn.Module):
    ...
    def __init__(
        ...
        self.activation = nn.LeakyReLU(0.2, False)#激活函数  正数输入保持不变，而负数输入则乘以0.2的斜率。
        self.norm_0 = SPADE(
            label_nc=label_nc,
        ...
        if self.learned_shortcut:
            ...
            self.norm_s = SPADE(
                label_nc=label_nc,#这里的label_nc是指输入的label的通道数
                norm_nc=self.in_channels,#这里的norm_nc是指输出的x的通道数
                kernel_size=kernel_size,
                spatial_dims=spatial_dims,
                hidden_channels=spade_intermediate_channels,
                norm=norm,
...
class SPADEEncoder(nn.Module):
    ...
    def forward(self, x):
        for block in self.blocks:
            x = block(x)
        x = x.view(x.size(0), -1)#第一个维度不变，后面自己计算
        mu = self.fc_mu(x)
    ...
    def reparameterize(self, mu, logvar):
        ...
        # tensor([[-1.4406, 1.5207, -0.4570],
        #         [-1.0709, 0.1118, 2.2785]])
...
class SPADEDecoder(nn.Module):
    ...
    def __init__(
        ...
        self.upsampling = torch.nn.Upsample(scale_factor=2, mode=upsampling_mode)#放大两倍
        for ch_ind, ch_value in enumerate(num_channels[:-1]):
            blocks.append(
                SPADEResNetBlock(
                    spatial_dims=spatial_dims,
                    in_channels=ch_value,#输入通道 16 32 64
                    out_channels=num_channels[ch_ind + 1], #输出通道 32 64 128
                    label_nc=label_nc,#标签通道 6
                    spade_intermediate_channels=spade_intermediate_channels,#中间层通道 128
                    norm=norm,
                    kernel_size=kernel_size,
                )
            )
