...
class CrossAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, context: torch.Tensor | None = None) -> torch.Tensor:
        query = self.to_q(x)
        # print("Cross attention query", x.shape,query.shape)
        context = context if context is not None else x
        # print('context',context.shape)
        key = self.to_k(context)
        # print("Cross attention key", context.shape, key.shape)
        value = self.to_v(context)
        # print("Cross attention value", context.shape, value.shape)

        # Multi-Head Attention
        query = self.reshape_heads_to_batch_dim(query)
        # print("Cross attention reshape query", query.shape)
        key = self.reshape_heads_to_batch_dim(key)
        # print("Cross attention reshape key", key.shape)
        value = self.reshape_heads_to_batch_dim(value)
        # print("Cross attention reshape value", value.shape)
        ...
        # print("Cross attention eshape_batch_dim_to_heads ", x.shape)
        x = x.to(query.dtype)
        # print("Cross attention eshape_batch_dim_to_heads ", x.shape)
...
class BasicTransformerBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, context: torch.Tensor | None = None) -> torch.Tensor:
        ...
        # print("Basictransformerattn1",x.shape)

        # 2. Cross-Attention
        x = self.attn2(self.norm2(x), context=context) + x
        # print("Basictransformerattn2", x.shape)

        # 3. Feed-forward
        x = self.ff(self.norm3(x)) + x
        # print("Basictransformerff", x.shape)
        return x
...
class SpatialTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, context: torch.Tensor | None = None) -> torch.Tensor:
        ...
        residual = x
        x = self.norm(x)
        # print("SpatialTransformernorm",x.shape)
        x = self.proj_in(x)
        # print("SpatialTransformerproj_in", x.shape)
        inner_dim = x.shape[1]
        ...
        for block in self.transformer_blocks:
            ...
            # print("SpatialTransformertransformer", x.shape)
        ...
        x = self.proj_out(x)
        # print("SpatialTransformerproj_out", x.shape)
        return x + residual
...
class AttentionBlock(nn.Module):
    ...
    def __init__(
        ...
        #计算注意力机制中的缩放因子，通常用于稳定训练过程。缩放因子是通道数除以头数的平方根的倒数。
    ...
    def reshape_heads_to_batch_dim(self, x: torch.Tensor) -> torch.Tensor:
        batch_size, seq_len, dim = x.shape
        #获取输入张量 x 的形状，并将其分解为三个部分：batch_size（批次大小）、seq_len（序列长度或特征图的尺寸）、dim（每个序列元素的特征维度）。
        x = x.reshape(batch_size, seq_len, self.num_heads, dim // self.num_heads)
        x = x.permute(0, 2, 1, 3).reshape(batch_size * self.num_heads, seq_len, dim // self.num_heads)#用于重新排列张量的维度。
        return x
    ...
    def _memory_efficient_attention_xformers(
        ...
        key = key.contiguous()
        value = value.contiguous()
        x = xformers.ops.memory_efficient_attention(query, key, value, attn_bias=None)#计算注意力权重和输出。
        return x
#  flash attention https://zhuanlan.zhihu.com/p/618533434
    def _attention(self, query: torch.Tensor, key: torch.Tensor, value: torch.Tensor) -> torch.Tensor:
        attention_scores = torch.baddbmm(
            torch.empty(query.shape[0], query.shape[1], key.shape[1], dtype=query.dtype, device=query.device),
        #函数计算查询和键的点积，得到注意力分数。torch.baddbmm
        # 是一个批量矩阵 - 矩阵乘法函数，它在计算时会累加到目标矩阵上，适合于这种累加点积的场景。beta = 0
        # 表示不累加，即从零开始计算；alpha = self.scale是缩放因子，用于稳定梯度，通常取值为1 / sqrt(d_k)，其中 d_k是键的维度。
            query,
            key.transpose(-1, -2),
            beta=0,
            alpha=self.scale,
        )
        attention_probs = attention_scores.softmax(dim=-1)#计算得到的注意力分数通过 softmax 函数转换为注意力概率分布。
        x = torch.bmm(attention_probs, value)#通过批量矩阵乘法计算加权的值，
        return x
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # norm
        x = self.norm(x)#对输入数据 x 应用组归一化。

        if self.spatial_dims == 2:
            x = x.view(batch, channel, height * width).transpose(1, 2)
            # 将原始张量x 重塑并转置，使得新的张量具有以下维度：
            # 0维：batch（批次大小）
            # 1维：height * width（将原始的高度和宽度合并）
            # 2维：channel（通道数）
        if self.spatial_dims == 3:
            x = x.view(batch, channel, height * width * depth).transpose(1, 2)

        # proj to q, k, v 使用线性层将输入 x 转换成查询（Query）、键（Key）和值（Value）。
        query = self.to_q(x)
        key = self.to_k(x)
        value = self.to_v(x)

        # Multi-Head Attention  调整查询、键和值的形状以适应多头注意力机制。
        query = self.reshape_heads_to_batch_dim(query)
        key = self.reshape_heads_to_batch_dim(key)
        value = self.reshape_heads_to_batch_dim(value)
...
class Downsample(nn.Module):
    ...
    def forward(self, x: torch.Tensor, emb: torch.Tensor | None = None) -> torch.Tensor:
        ...
        # print("Downsampling...",x.shape,self.num_channels,self.out_channels)
        return self.op(x)
...
class ResnetBlock(nn.Module):
    ...
    def __init__(
        ...
        # 这使得SiLU在一些应用中比ReLU表现更好，例如在语音识别中使用SiLU比ReLU可以取得更好的效果。
    ...
    def forward(self, x: torch.Tensor, emb: torch.Tensor) -> torch.Tensor:
        h = x
        h = self.norm1(h)
        # print("ResnetBlock forwardnorm1",h.shape)
        h = self.nonlinearity(h)
        # print("ResnetBlock forwardnonlinearity", h.shape)
        if self.upsample is not None:
            if h.shape[0] >= 64:
                x = x.contiguous()#确保张量（tensor）在内存中是连续存储的方法。
                h = h.contiguous()
            x = self.upsample(x)
            h = self.upsample(h)
        ...
        h = h + temb
        # print("ResnetBlock forwardtemb", h.shape)
        h = self.norm2(h)
        # print("ResnetBlock forwardtnorm2", h.shape)
        h = self.nonlinearity(h)
        # print("ResnetBlock forwardnonlinearity", h.shape)
        h = self.conv2(h)
        # print("ResnetBlock forwardtconv2", h.shape)
...
class CrossAttnDownBlock(nn.Module):
    ...
    def __init__(
        ...
        cross_attention_dim: int | None = None,
        upcast_attention: bool = False,#是否将注意力机制中的操作提升到更高的精度。
        use_flash_attention: bool = False,
...
class DiffusionModelUNet(nn.Module):
    ...
    Args:
        ...
        num_channels: tuple of block output channels.#一个元组，指定了每个残差块的输出通道数。
        attention_levels: list of levels to add attention.
        norm_num_groups: number of groups for the normalization.
        norm_eps: epsilon for the normalization.#规范化操作中使用的epsilon值，用于数值稳定性。
        resblock_updown: if True use residual blocks for up/downsampling.#如果为True，使用残差块进行上采样和下采样操作。
        num_head_channels: number of channels in each attention head.#每个注意力头的通道数
        with_conditioning: if True add spatial transformers to perform conditioning.#如果为True，添加空间变换器（Spatial Transformers）以执行条件操作。
        transformer_num_layers: number of layers of Transformer blocks to use.#要使用的Transformer块的层数。
        cross_attention_dim: number of context dimensions to use.#要使用的交叉注意力的上下文维度数。
        num_class_embeds: if specified (as an int), then this model will be class-conditional with `num_class_embeds`
        classes.
        #如果指定（作为一个整数），则该模型将是类条件模型，具有 num_class_embeds 类的嵌入。
        upcast_attention: if True, upcast attention operations to full precision.#如果为True，将注意力操作上转换为全精度。
        use_flash_attention: if True, use flash attention for a memory efficient attention mechanism.
        #如果为True，使用flash注意力机制，这是一种节省内存的注意力机制。
        dropout_cattn: if different from zero, this will be the dropout value for the cross-attention layers
        #如果不等于零，这将是交叉注意力层的dropout值。
    """
    ...
    def __init__(
        ...
        if isinstance(num_head_channels, int):
            ...
            # 根据 attention_levels 列表的长度，将 num_head_channels 的值重复多次并形成一个元组。
...
#第一个全连接层，将输入从 num_channels[0] 维映射到 time_embed_dim 维。
        #SiLU（Sigmoid-like activation function）激活函数，也称为 Swish，由谷歌提出，用于在神经网络中引入非线性。
        #第二个全连接层，将数据从time_embed_dim维映射回time_embed_dim维。这种设计有时用于增加层间的特征表示能力。
        # class embedding
        ...
        #用于存储多个神经网络层（在这里是下采样块），并且可以确保其中的层在模型训练时被正确注册和更新。
        ...
        for i in range(len(num_channels)):
            ...
            down_block = get_down_block(
                ...
                norm_num_groups=norm_num_groups,
                norm_eps=norm_eps,
                add_downsample=not is_final_block,#如果不是最后一个块，则添加下采样操作以降低特征图的空间分辨率。
                resblock_updown=resblock_updown,
                with_attn=(attention_levels[i] and not with_conditioning),
                with_cross_attn=(attention_levels[i] and with_conditioning),
                num_head_channels=num_head_channels[i],
                transformer_num_layers=transformer_num_layers,
                cross_attention_dim=cross_attention_dim,#定义了交叉注意力的维度。
                upcast_attention=upcast_attention,
                use_flash_attention=use_flash_attention,
                dropout_cattn=dropout_cattn,
            )
    ...
    def forward(
        ...
        if self.num_class_embeds is not None:
            ...
            emb = emb + class_emb
        # print("num_class_embeds",self.num_class_embeds)
        # 3. initial convolution
        # print("model-int",x.shape)
        h = self.conv_in(x)
        ...
        for downsample_block in self.down_blocks:
            ...
            # print("model-downsample_block", h.shape,len(res_samples),len(down_block_res_samples))
        ...
        # print("model-middle_block", h.shape)
        ...
        for upsample_block in self.up_blocks:
            ...
            # print("model-upsample_block", h.shape)
