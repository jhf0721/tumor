        ...
        for i_ in range(self.num_d):
            ...
            if i_ == 0 or pool is None:
                subnet_d = PatchDiscriminator(
                    ...
                    num_channels=self.num_channels,#通道数
                    in_channels=in_channels,
                    out_channels=out_channels,
                    num_layers_d=num_layers_d_i,#层数
                    ...
                    dropout=dropout,
                    last_conv_kernel_size=last_conv_kernel_size,#最后一层卷积核大小
                )
            else:
...
class PatchDiscriminator(nn.Sequential):
    ...
    def forward(self, x: torch.Tensor) -> list[torch.Tensor]:
        ...
        """
        out = [x]
        for submodel in self.children():#遍历模型中所有的子模块。
            intermediate_output = submodel(out[-1])
            out.append(intermediate_output)

        return out[1:]#除去原始输入的所有中间层输出和最终输出。
