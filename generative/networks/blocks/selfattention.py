...
#16 256 96
        ...
        query = query.view(b, t, self.num_heads, c // self.num_heads)  # (b, t, nh, hs)16 256 8 12
        ...
        if self.use_flash_attention:
            ...
            query = query.transpose(1, 2)  # (b, nh, t, hs)#转置
            key = key.transpose(1, 2)  # (b, nh, kv_t, hs)
            value = value.transpose(1, 2)  # (b, nh, kv_t, hs)

            # manual implementation of attention
            query = query * self.scale#缩放
            attention_scores = query @ key.transpose(-2, -1)#矩阵乘法 点积
