...
class DiffusionInferer(Inferer):
    ...
    def __call__(
        ...
        #如果模型是 SPADEDiffusionModelUNet 的一个实例，则此参数指定必须提供分割信息。这对于使用空间注意力或离散归一化流的模型尤其重要，它们需要此信息来进行前向传播。
    ) -> tuple[torch.Tensor]:
        """
        ...
        noisy_image, square_alphas_t,square_1_alphas_t = self.scheduler.add_noise(original_samples=inputs, noise=noise, timesteps=timesteps)
        timesteps_1=timesteps-1
        noisy_image_1,_,_ =self.scheduler.add_noise(original_samples=inputs, noise=noise, timesteps=timesteps_1)
        noise_1=(noisy_image-square_alphas_t*noisy_image_1)/square_1_alphas_t
        #使用 scheduler 的 add_noise 方法向原始样本 inputs 添加噪声 noise。
        if mode == "concat":
        ...
        print("inferer",noisy_image.shape)
        prediction = diffusion_model(x=noisy_image, timesteps=timesteps, context=condition)
        #使用准备好的扩散模型 diffusion_model，传入噪声图像 noisy_image、时间步 timesteps 和上下文 condition 进行预测。

        # return prediction,noise_1
...
#用于从给定的扩散模型中采样数据
    ...
    def sample(
        ...
        for t in progress_bar:
            ...
            #如果 mode 是 "concat"，则将 image 和 conditioning 在第1维（通常是通道维）上进行拼接。否则，image 直接作为模型的输入。
            if mode == "concat":
...
#用于计算输入数据的对数似然性（log-likelihood）。
    ...
    def get_likelihood(
        ...
        #创建与输入数据 inputs 形状相同的随机噪声 noise。
        total_kl = torch.zeros(inputs.shape[0]).to(inputs.device)
        for t in progress_bar:
            timesteps = torch.full(inputs.shape[:1], t, device=inputs.device).long()
            noisy_image = self.scheduler.add_noise(original_samples=inputs, noise=noise, timesteps=timesteps)#添加噪声
            diffusion_model = (
                partial(diffusion_model, seg=seg)
                if isinstance(diffusion_model, SPADEDiffusionModelUNet)
                else diffusion_model
            ...
            #根据 scheduler.prediction_type 的不同，代码使用不同的方法计算预测的原始样本 pred_original_sample。
            ...
            #这行代码计算后验方差的对数值。后验方差是从模型的后验分布中得到的，它表示了在给定观测数据和先验知识下，模型参数的不确定性。
            log_predicted_variance = torch.log(predicted_variance) if predicted_variance else log_posterior_variance
#通过对数方差，可以更稳定地处理方差值，特别是在方差值可能非常小的情况下。此外，对数方差在计算KL散度时可以直接用于指数族分布的变分推断，这在扩散模型的上下文中是很有用的。
            if t == 0:
                # compute -log p(x_0|x_1)
                #计算高斯分布在离散化到给定图像时的对数似然。
    ...
    def _get_decoder_log_likelihood(
        ...
        centered_x = inputs - means#将输入数据 inputs 减去 means 进行中心化。
        inv_stdv = torch.exp(-log_scales)#计算 log_scales 的指数，得到高斯分布的逆标准差。
        #计算累积分布函数 (CDF)：
        ...
        #根据输入值与缩放范围的关系，计算对数概率。使用 torch.where 来处理输入值在不同区间的情况。
...
class VQVAETransformerInferer(Inferer):
    ...
    def __call__(
        ...
        latent_spatial_dim = tuple(latent.shape[1:])#获取 latent 张量的空间维度，并将其存储为一个元组。
        print(latent_spatial_dim)
        latent = latent.reshape(latent.shape[0], -1)
        print(latent.shape)
        #第一个维度的大小保持不变，即原始张量的第一个维度大小（批次大小或序列长度）。
        # 第二个维度的大小是自动计算的，以确保重塑后的张量包含与原始张量相同数量的元素。
        latent = latent[:, ordering.get_sequence_ordering()]
        print(latent.shape)

        #使用 ordering.get_sequence_ordering() 函数获取顺序编码的序列，并使用它重塑 latent 张量。
        ...
        #使用 F.pad 函数在 latent 张量的左侧添加一个填充，使用 vqvae_model.num_embeddings 作为填充值，
        # crop the last token as we do not need the probability of the token that follows it
        latent = latent[:, :-1]#去除 latent 张量的最后一列，因为我们不需要填充标记的概率。
        print(latent.shape)
        latent = latent.long()

        # train on a part of the sequence if it is longer than max_seq_length
        seq_len = latent.shape[1]#获取 latent 张量的序列长度。
