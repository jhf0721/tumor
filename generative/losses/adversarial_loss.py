...
class PatchAdversarialLoss(_Loss):
    ...
    def forward(
        ...
        for _, disc_out in enumerate(input):
            if self.criterion != AdversarialCriterions.HINGE.value:#即是否使用Hinge损失
                target_.append(self.get_target_tensor(disc_out, target_is_real))#使用 get_target_tensor 方法生成目标张量
            else:
    ...
    def forward_single(self, input: torch.FloatTensor, target: torch.FloatTensor) -> torch.Tensor | None:
        if (
            self.criterion == AdversarialCriterions.BCE.value#二元交叉熵（BCE）
            or self.criterion == AdversarialCriterions.LEAST_SQUARE.value#最小二乘（Least Square）损失
        ):
            return self.loss_fct(input, target)
        elif self.criterion == AdversarialCriterions.HINGE.value:#铰链（Hinge）损失:
