...
parser.add_argument('--log_dir', default="runs/synt.no_pretrain.swin_unetrv2_tiny_no_examine", type=str)
...
def main():
    ...
    with torch.no_grad():
        for idx, val_data in enumerate(val_loader):
            ...
            # breakpoint()
            # val_data['label']
            # x=val_data['label'].clone()
            # val_data['label'][x>1.5] = 2
            # condition = (val_data['label'] < 1.5) & (val_data['label'] > 0.5)
            # val_data['label'][condition] = 1
